...
const Card = ({ title, heading, paragraph, l1, l2, l3 }) => {
  return (
    <div className="backdrop-blur-md border border-white/15 space-y-2 p-4 md:p-8 rounded-lg max-h-fit max-w-[400px]">
      <h3 className="text-green text-sm uppercase font-semibold">{title}</h3>
      <h2 className="w-24 text-2xl md:text-3xl font-semibold">{heading}</h2>
      <p className="text-white/75 text-base md:text-lg">{paragraph}</p>
